...
Planner version 8 completes that structural prerequisite. A content-free dry
run over the same 65-deck, 1,111-Slide snapshot converts all 161 canonical stack
Slides across nine decks to `presenter/stack`; no canonical stack remains. The
158 previously ineligible records carried only `slide-{number}` wrapper classes
generated by Presenter's historical importer. Stack now validates, stores, and
renders those classes unchanged on Reveal's outer stack section instead of
discarding them. The eight opaque nested wrappers remain exact Custom HTML
because their additional section layer is structurally ambiguous.

The subsequent pristine 65-deck rehearsal migrated and exactly restored every
deck with zero runner failures. Its structural report returned to the accepted
baseline of 57 passes, the three intentional native Chart conversion
differences, and five access skips. All 161 converted stacks matched legacy
rendered content, fragments, notes, wrapper classes, data attributes, hierarchy,
and anchors. Conversion preserves each child section ID exactly—including no ID
and a parent-colliding ID—rather than manufacturing or uniquifying an existing
Reveal address.
