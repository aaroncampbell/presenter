...
| Migration apply | Exact conditional content write; shared native Deck validation; renewed-lock and editor/source checks before singleton native cutover; retained legacy metadata; crash resume before/after cutover; owned-marker compensation; inverse conditional rollback; recovery-required state; content-free/idempotent CLI | Automated PHP race/failure integration and real WP-CLI apply gate; restore remains pending |
