...
| Accessibility safeguards | Escaped Slide-label `aria-label`, visible hidden-Slide treatment, `aria-invalid` background feedback, controls-or-keyboard invariant, keyboard-tested skip link and focusable target, and one plugin-owned zoom-capable viewport declaration in both standalone templates | Automated PHP, JavaScript, and real WordPress headless runtime |
...
| Migration dry-run | Complete immutable source snapshot, site-keyed fingerprint, stable filter-independent ordering, exact redundant-data collapse, characterized no-op source shapes, historical 960×700 sizing, registered legacy-theme aliases, exact Slide class/data mapping, lossless HTML-note formats, canonical and opaque nested-section compatibility, content-free blockers, bounded CLI batches, and zero writes | PHP/JavaScript integration, real WP-CLI synthetic gate, and isolated 65-deck corpus report with 65 ready plans including the password-protected deck |
...
| Migration preparation and status | Explicit single-deck preparation; active editor/lock exclusion; exact verified revision; immutable source backup; renewed-lock source recheck; verified `apply_prepared` event; exact idempotent retry; recursively content-free, zero-write status | Automated PHP integration and real WP-CLI synthetic gate; no content, metadata, or route cutover |
| Migration apply and restore | Exact conditional content writes; shared native Deck validation; renewed-lock and editor/source checks; singleton native cutover and exact conditional removal; retained legacy metadata; crash resume across apply and restore representations; owned-marker compensation; inverse conditional rollback; recovery-required state; content-free/idempotent CLI | Automated PHP race/failure integration and real WP-CLI apply/restore gates |
