...
legacy decks without displaying their authored source. Eligible untrusted decks
sort first. An administrator may select one page (including **Select all**) or
use the counted **Trust all** action, confirm review, and trust the exact current
slide sequences. The server rechecks the nonce, both capabilities, post type,
active legacy representation, requested scope, and current stored slides before
it writes each content-bound marker. Native cutovers and already-trusted content
...
label, group anchor, and validated wrapper classes that apply unchanged to the
outer Reveal stack section, but no other behavior or notes that would be lost on
the container. This preserves `slide-{number}` classes generated by Presenter's
historical importer. Every direct child must be a `section` whose ID, classes,
and `data-*` attributes map exactly to a native Presenter Slide. The planner
preserves each child ID exactly, including an absent ID or one duplicated by
the outer stack, so existing Reveal addresses do not change. It reports
